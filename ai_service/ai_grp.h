...
public:
	...
	std::string getEventParamString(uint32 i)
	{
		if (i >= _EventParams.size()) return "";
		return _EventParams[i];
	}
	float getEventParamFloat(uint32 i)
	{
		if (i >= _EventParams.size()) return 0.0f;
		return (float)atof(_EventParams[i].c_str());
	}
...
inline CSpawnGroup::CSpawnGroup(CPersistent<CSpawnGroup> &owner)
...
inline void CSpawnGroup::setMoveProfileFromStateMachine(IAIProfileFactory *staticProfile)
{
	_MovingProfile.setAIProfile(this, staticProfile, true);
}

inline void CSpawnGroup::setActivityProfileFromStateMachine(IAIProfileFactory *staticProfile)
{
	_ActivityProfile.setAIProfile(this, staticProfile, true);
}

inline bool CSpawnGroup::isGroupAlive(uint32 const nbMoreKilledBot) const
{
	return ((sint32)_NbSpawnedBot - (sint32)_BotsToDespawn.size() - (sint32)nbMoreKilledBot) > 0;
}

inline CGroup &CSpawnGroup::getPersistent() const
{
	return static_cast<CGroup &>(CSpawnable<CPersistent<CSpawnGroup>>::getPersistent());
}

inline CAliasCont<CBot> const &CSpawnGroup::bots() const
{
	return getPersistent()._Bots;
}

inline CAliasCont<CBot> &CSpawnGroup::bots()
{
	return getPersistent()._Bots;
}
...
inline void CGroup::despawnBots(bool immediately)
{
	if (!isSpawned())
		return;
...
inline CBot *CGroup::getBot(uint32 index)
{
	if (index >= _Bots.size())
		return NULL;
	return _Bots[index];
