...
public:
	...
	explicit CAIPos(CAIVector const &xy, sint32 h, float theta)
	    : CAIVector(xy)
	    , _h(h)
	    , _theta(theta)
	{
	}
	explicit CAIPos(CAIVector const &xy, sint32 h, CAngle const &theta)
	    : CAIVector(xy)
	    , _h(h)
	    , _theta(theta)
	{
	...
	template <class C>
	explicit CAIPos(C x, C y, sint32 h, float theta)
	    : CAIVector(x, y)
	    , _h(h)
	    , _theta(theta)
	{
	...
	template <class C>
	explicit CAIPos(C x, C y, sint32 h, CAngle theta)
	    : CAIVector(x, y)
	    , _h(h)
	    , _theta(theta)
	{
	...
	inline const CAIPos &operator=(const CAIPos &other)
	{
		setX(other.x());
		setY(other.y());
		_h = other._h;
	...
	template <class V>
	inline const CAIPos &operator+=(const V &v)
	{
		CAIVector::operator+=(v);
		return *this;
	}
	template <class V>
	inline const CAIPos &operator-=(const V &v)
	...
	template <class V>
	inline const CAIPos operator+(const V &v) const
	{
		CAIPos p(*this);
		return p += v;
	}
	...
	template <class C>
	inline void setH(const C &h) { _h = h; }
	template <class C>
	inline void setTheta(const C &theta) { _theta = theta; }
