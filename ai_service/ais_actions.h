...
#define DEFINE_ACTION_TEMPLATE1(context, cmdName, templateTypeName1)                 \
	...
	{                                                                                \
		...
		CActionHandler_##context##_##cmdName(char const *name)                       \
		    : CAISActions::IActionHandler(CAISActionEnums::context, name)            \
		    , _Name(name)                                                            \
		{                                                                            \
		}                                                                            \
    ...
    const char *actionName, T0 &v0, T1 &v1, T2 &v2, T3 &v3, T4 &v4, T5 &v5, T6 &v6)
{
	if (args.size() != 7)
	{
		nlwarning("Action %s FAILED due to argument count", actionName);
		return false;
	}
	if (!args[0].get(v0))
	{
		nlwarning("Action %s FAILED due to type of argument 0", actionName);
		return false;
	}
	if (!args[1].get(v1))
	{
		nlwarning("Action %s FAILED due to type of argument 1", actionName);
		return false;
	}
	if (!args[2].get(v2))
	{
		nlwarning("Action %s FAILED due to type of argument 2", actionName);
		return false;
	}
	if (!args[3].get(v3))
	{
		nlwarning("Action %s FAILED due to type of argument 3", actionName);
		return false;
	}
	if (!args[4].get(v4))
	{
		nlwarning("Action %s FAILED due to type of argument 4", actionName);
		return false;
	}
	if (!args[5].get(v5))
	{
		nlwarning("Action %s FAILED due to type of argument 5", actionName);
		return false;
	}
	if (!args[6].get(v6))
	{
		nlwarning("Action %s FAILED due to type of argument 6", actionName);
		return false;
	}
	return true;
}
