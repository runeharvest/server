...
inline CPopulationRecord::CPopulationRecord(AISHEETS::ICreatureCPtr const &sheetData, uint count)
    : _SheetData(sheetData)
    , _Count(count)
{
}

inline bool CPopulationRecord::operator==(CPopulationRecord const &other) const
{
	return (_Count == other._Count && _SheetData == other._SheetData);
}

inline AISHEETS::ICreatureCPtr CPopulationRecord::getCreatureSheet() const
{
	return _SheetData;
}

inline uint CPopulationRecord::getBotCount(bool useCreatureMultiplier) const
{
...
inline uint32 CPopulationRecord::getEnergyValue(bool useCreatureMultiplier) const
{
	if (_SheetData)
	{
...
inline CPopulation::CPopulation(CPopulationOwner *owner, CAIAliasDescriptionNode *aliasDescription)
...
inline CPopulation::CPopulation(CPopulationOwner *owner, uint32 alias, std::string name)
...
inline void CPopulation::addPopRecord(CPopulationRecord popRecord)
{
...
*/
inline void CAabb::includeAabb(CAabb const &box)
{
...
inline bool CFaunaZone::haveActivity(AITYPES::CPropertyId const &activity) const
{
...
inline bool CFaunaZone::haveActivity(AITYPES::CPropertySet const &activities) const
{
	if (!_AdditionalActivities.empty())
...
inline float CFaunaZone::getFreeAreaScore() const
{
	float radius = getRadius();
	return (float)((radius * radius) / ((float)getNbUse() + 0.001));
}

inline uint32 CFaunaZone::getNbUse() const
{
...
inline CAIRefPlaceXYR::CAIRefPlaceXYR(CPlaceOwner *owner, CAIPlace const *zone)
    : CAIPlace(owner, NULL)
{
#ifdef NL_DEBUG
	nlassert(zone != NULL);
...
inline CAIRefPlaceXYR::operator CAIPlace const *() const
{
	return _Zone;
}
...
inline CAIRefPlaceXYRFauna::CAIRefPlaceXYRFauna(CPlaceOwner *owner, CAIPlace const *zone)
...
inline CRebuildContinentAndOutPost::CRebuildContinentAndOutPost(CContinent *continent)
    : _Continent(continent)
{
}

inline bool CRebuildContinentAndOutPost::absorb(CLazyProcess const &lazyProcess) const
{
...
inline bool CRebuildContinentAndOutPost::operator==(CRebuildContinentAndOutPost const &other) const
{
...
inline CContinent::CContinent(CAIInstance *owner)
...
inline void CRoad::setDifficulty(float const &difficulty)
{
	_Difficulty = difficulty;
	calCost();
}

inline void CRoad::calCost()
{
	_CostCoef = _Length * _Difficulty;
}

inline float CRoad::getCost() const
{
...
inline CRoadTrigger::CRoadTrigger(CRoad *owner, uint32 alias, std::string const &name)
...
inline size_t CCell::npcZoneCount()
{
	return _NpcZonePlaces.size() + _NpcZoneShapes.size();
}

inline CNpcZone *CCell::npcZone(size_t index)
{
...
inline void CCell::getNeighBourgCellList(std::vector<CCell *> &cells) const
{
...
inline CGroupFamily::~CGroupFamily()
{
	_GroupDescs.clear();
}

inline void CGroupFamily::setProfileParams(std::string const &str, NLMISC::CVirtualRefCount *objet)
{
#if !FINAL_VERSION
	nlassert(objet);
#endif
...
inline NLMISC::CVirtualRefCount const *CGroupFamily::getProfileParams(std::string const &str) const
{
...
inline void CGroupFamily::addProfileProperty(std::string const &propertyName, AITYPES::CPropertySet const &property)
{
	_Properties[NLMISC::CStringMapper::map(propertyName)] = property;
}

inline AITYPES::CPropertySet const &CGroupFamily::getProfileProperty(std::string const &propertyName) const
{
...
inline void CCell::unrefZoneInRoads()
{
...
inline void CNpcZone::unrefZoneInRoads()
{
	while (!_Roads.empty())
...
inline void CCellZone::unrefZoneInRoads()
{
