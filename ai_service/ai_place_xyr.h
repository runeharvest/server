...
public:
	...
	CFaunaGenericPlace();
	// activities flags
	void setFlag(TFlag flag, bool on)
	{
		nlassert(flag < FLAG_COUNT);
		_Flags[flag] = on;
	}
	bool getFlag(TFlag flag) const
	{
		nlassert(flag < FLAG_COUNT);
		return _Flags[flag];
	}
...
inline CAIPlaceXYR::CAIPlaceXYR(CPlaceOwner *owner, CAIAliasDescriptionNode *aliasDescription)
    : CAIPlace(owner, aliasDescription)
{
}

inline bool CAIPlaceXYR::atPlace(CAIVector const &pos) const
{
	return (pos - _pos).sqrnorm() <= ((double)_radius * (double)_radius);
}

inline bool CAIPlaceXYR::atPlace(CAIVectorMirror const &pos) const
{
	return (pos - _pos).sqrnorm() <= ((double)_radius * (double)_radius);
}

inline void CAIPlaceXYR::setPosAndRadius(AITYPES::TVerticalPos verticalPos, CAIPos const &pos, uint32 radius)
{
...
inline CAIPlaceXYR &CAIPlaceXYR::operator=(CAIPlaceXYR const &other)
{
...
inline AITYPES::TVerticalPos CAIPlaceXYR::getVerticalPos() const
{
	return CPlaceRandomPos::getVerticalPos();
}

inline void CAIPlaceXYR::getRandomPos(RYAI_MAP_CRUNCH::CWorldPosition &pos) const
{
	CPlaceRandomPos::getRandomPos(pos);
}

inline bool CAIPlaceXYR::calcRandomPos(CAIPos &pos) const
{
	double dx, dy;
...
// CFaunaGenericPlace //
////////////////////////
inline CFaunaGenericPlace::CFaunaGenericPlace()
{
...
inline CAIPlaceXYRFauna::CAIPlaceXYRFauna(CPlaceOwner *owner, CAIAliasDescriptionNode *aliasDescription)
...
inline CAIPlaceFastXYR::CAIPlaceFastXYR(CPlaceOwner *owner, CAIAliasDescriptionNode *aliasDescription)
    : CAIPlace(owner, aliasDescription)
{
}

inline bool CAIPlaceFastXYR::atPlace(CAIVector const &pos) const
{
	return (float)((pos - _Pos).sqrnorm()) <= (_Radius * _Radius);
}

inline bool CAIPlaceFastXYR::atPlace(CAIVectorMirror const &pos) const
{
	return (float)((pos - _Pos).sqrnorm()) <= ((double)_Radius * (double)_Radius);
}

inline void CAIPlaceFastXYR::setPosAndRadius(AITYPES::TVerticalPos verticalPos, CAIPos const &pos, uint32 radius)
{
#ifdef NL_DEBUG
	nlassert(radius > 0);
	nlassert(pos.x() != 0 || pos.y() != 0);
...
inline CAIPlaceFastXYR &CAIPlaceFastXYR::operator=(CAIPlaceFastXYR const &other)
{
...
inline AITYPES::TVerticalPos CAIPlaceFastXYR::getVerticalPos() const
{
	return _VerticalPos;
}

inline void CAIPlaceFastXYR::getRandomPos(RYAI_MAP_CRUNCH::CWorldPosition &wpos) const
{
	CAIPos pos;
...
inline bool CAIPlaceFastXYR::calcRandomPos(CAIPos &pos) const
{
	double dx, dy;
	double const r = (double)_Radius;
	double const rSquare = r * r;
...
inline RYAI_MAP_CRUNCH::CWorldPosition const &CAIPlaceFastXYR::worldValidPos() const
{
