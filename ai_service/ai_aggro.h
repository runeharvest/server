...
public:
	...
	{
		if (_LastHitPlace) return _LastHitPlace->atPlace(pos);
		else return true;
	}
	...
	{
		if (_LastHitPlace) return _LastHitPlace->atPlace(entity);
		else return true;
	}
...
private:
	virtual bool atPlace(CAIVector const &pos) const
	{
		if (_FirstHitPlace) return _FirstHitPlace->atPlace(pos);
		else return true;
	}
	virtual bool atPlace(CAIVectorMirror const &pos) const
	{
	...
	virtual bool atPlace(CAIEntityPhysical const *entity) const
	{
		if (_FirstHitPlace) return _FirstHitPlace->atPlace(entity);
		else return true;
	}
