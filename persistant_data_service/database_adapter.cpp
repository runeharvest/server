...
bool CDatabaseAdapter::buildColumnContent(const CColumnAdapter &adapter, const uint8 *from, uint8 *into)
{
	...
	case PDS_CSheetId:
		return adaptCSheetId(adapter, fromObject, intoObject);
		break;
...
// simple data cast
#define ADAPT_DATA(typefrom, typeinto) \
	case PDS_##typefrom: (*(typeinto *)into) = (typeinto)(*(typefrom *)from); return true;
