...
uint32 CMissionTemplate::testPrerequisits(CCharacter *user, CPrerequisitInfos &prereqInfos, bool logOnFail, bool fillPrereqInfos) const
{
	...
	if (Type == MISSION_DESC::Group)
	{
		...
		else
		{
			...
			{
				if ((team->getMissions()[j]->getTemplateId() == alias) || (team->getMissions()[j]->getMainMissionTemplateId() == alias))
				{
					if (logOnFail)
	...
	else if (Type == MISSION_DESC::Guild)
	{
		...
		{
			if (guild->getMissions()[j]->getTemplateId() == alias || guild->getMissions()[j]->getMainMissionTemplateId() == alias)
			{
