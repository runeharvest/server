...
static const struct
{
	...
	// CSR variables
	"Invisible",
	true, // not in /who; hierarchy except for user with privilege
	"God",
	true,
	"Invulnerable",
	true,
	...
	"outpostUnbanPlayer",
	false,
	"outpostChallengeByGuild",
	false,
	"outpostSimulateTimer0End",
	false,
	"outpostSimulateTimer1End",
	false,
	"outpostSimulateTimer2End",
	false,
	"outpostSetFightData",
	false,
	"setOutpostLevel",
	false,
	"outpostAccelerateConstruction",
	false,
	"outpostSetOutpostOwner",
	false,
	"outpostChallengeOutpost",
	true,
	"outpostGiveupOutpost",
	true,
	"outpostDisplayGuildOutposts",
	true,
	"outpostForceOpenGuildInventory",
	true,
	"outpostForceCloseGuildInventory",
	true,
	"outpostSetPlayerPvpSide",
	true,
	"outpostSetOutpostToPlayer",
	true,
	"outpostSelectOutpost",
	true,
	"outpostUnselectOutpost",
	true,

	"setPvpClan",
	true, // remove it when interface fo choose pvp clan is ready
	"setPvPTag",
	true, // set pvp tag to true or false
...
NLMISC_COMMAND(webExecCommand, "Execute a web command", "<user id> <web_app_url> <index> <command> <hmac> [<new_check=0|1|2|3>] [<next_step=0|1>] [<send_url=0|1|2>]")
{
	...
	else if (command_args[0] == "check_outpost")
	{
		...
		if ((command_args[2] == "attacker" && (outpost->getAttackerGuild() == 0 || outpost->getAttackerGuild() != c->getGuildId())) || (command_args[2] == "defender" && (outpost->getOwnerGuild() == 0 || outpost->getOwnerGuild() != c->getGuildId())) || (command_args[2] == "attack" && outpost->getState() != OUTPOSTENUMS::AttackRound) || (command_args[2] == "defend" && outpost->getState() != OUTPOSTENUMS::DefenseRound))
		{
			if (send_url)
...
NLMISC_COMMAND(deleteInventoryItem, "Delete an item from a characters inventory", "<eid> <inventory> <slot> <sheetname> <quality> <quantity>")
{
	...
	if (itemPtr != NULL)
	{
		if (itemPtr->getSheetId().toString() == sheet_name && itemPtr->quality() == quality && itemPtr->getStackSize() >= quantity)
		{
