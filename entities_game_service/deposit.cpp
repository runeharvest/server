...
bool CDeposit::build(const NLLIGO::CPrimZone *zone)
{
	...
	*((NLLIGO::CPrimZone *)this) = *zone;
	...
	if (!(s1 && s2 && s3 && s4))
	{
		if (s1)
		{
			_SeasonFilter.push_back(EGSPD::CSeason::Spring);
			_FilterPhase |= 0x1;
		}
		if (s2)
		{
			_SeasonFilter.push_back(EGSPD::CSeason::Summer);
			_FilterPhase |= 0x2;
		}
		if (s3)
		{
			_SeasonFilter.push_back(EGSPD::CSeason::Autumn);
			_FilterPhase |= 0x4;
		}
		if (s4)
		{
			_SeasonFilter.push_back(EGSPD::CSeason::Winter);
			_FilterPhase |= 0x8;
		}
	}
	...
	if (!(w1 && w2 && w3 && w4))
	{
		if (w1)
		{
			_WeatherFilter.push_back(CRyzomTime::best);
			_FilterPhase |= 0x10;
		}
		if (w2)
		{
			_WeatherFilter.push_back(CRyzomTime::good);
			_FilterPhase |= 0x20;
		}
		if (w3)
		{
			_WeatherFilter.push_back(CRyzomTime::bad);
			_FilterPhase |= 0x40;
		}
		if (w4)
		{
			_WeatherFilter.push_back(CRyzomTime::worst);
			_FilterPhase |= 0x80;
		}
	}
...
void CDeposit::selectRMsByFilters(std::vector<std::string> &exactRMCodesS, const std::vector<std::string> &rmFamilyFilterS, const std::vector<std::string> &itemPartsFilterS, const std::vector<std::string> &craftCivS, uint minEnergy, uint maxEnergy)
{
	...
	for (vector<const CStaticItem *>::const_iterator it = sortedItems.begin(); it != sortedItems.end(); ++it)
	{
		...
		if (find(exactRMCodesS.begin(), exactRMCodesS.end(), sheetName) == exactRMCodesS.end())
		{
			...
			if ((_Ecotype != ECOSYSTEM::common_ecosystem) && (staticItem.Mp->Ecosystem != ECOSYSTEM::common_ecosystem) && (staticItem.Mp->Ecosystem != _Ecotype))
			{
			...
			if (find(rmFamilyFilter.begin(), rmFamilyFilter.end(), staticItem.Mp->Family) == rmFamilyFilter.end())
			{
				...
				for (vector<uint>::iterator ipf = itemPartsFilter.begin(); ipf != itemPartsFilter.end(); ++ipf)
				{
					...
					if (mpFaberParam && (mpFaberParam->Durability != 0))
					{
						...
						else if (VerboseDepositFiltering) nldebug("-CivSpec %s", ITEM_ORIGIN::enumToString(mpFaberParam->CraftCivSpec).c_str());
					}
					else
					{
...
public:
	///
	CIndexNoiseValue(uint arraySize, uint phase, float freq)
	    : CNoiseValue(0.0f, 1.0f, freq)
	    , _MaxIndex((float)(arraySize))
	    , _Phase(1210.191f * phase, 523.8883f * phase, 403.57614f * phase)
	{
	}
