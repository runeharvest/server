				...
				// apply procs
				{
					...
					for (it = effects.begin(), itEnd = effects.end(); it != itEnd; ++it)
					{
						...
						if (rnd < it->EffectArgFloat[0])
						{
							...
							else if (type == ITEM_TYPE::HEAVY_BOOTS || type == ITEM_TYPE::HEAVY_GLOVES || type == ITEM_TYPE::HEAVY_PANTS || type == ITEM_TYPE::HEAVY_SLEEVES || type == ITEM_TYPE::HEAVY_VEST || type == ITEM_TYPE::HEAVY_HELMET)
							{
								scoresAllowed[SCORES::focus] = false;
								scoresAllowed[SCORES::sap] = false;
							}
							else if (type == ITEM_TYPE::MEDIUM_BOOTS || type == ITEM_TYPE::MEDIUM_GLOVES || type == ITEM_TYPE::MEDIUM_PANTS || type == ITEM_TYPE::MEDIUM_SLEEVES || type == ITEM_TYPE::MEDIUM_VEST)
							{
								scoresAllowed[SCORES::focus] = false;
							}
