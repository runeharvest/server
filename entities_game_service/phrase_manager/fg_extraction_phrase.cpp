		...
		if (_Source)
		{
			...
			if (staticItem && staticItem->Mp)
			{
				...
				if ((fam >= FirstEp2Family) && (fam <= LastEp2Family))
				{
					...
					const SKILLS::ESkills skillsToTest[NbSkillsToTest] = { SKILLS::SHFDAEM, SKILLS::SHFFAEM, SKILLS::SHFJAEM, SKILLS::SHFLAEM, SKILLS::SHFPAEM };
...
bool CFgExtractionPhrase::validate()
{
	...
	if ((_RequestedProps[CHarvestSource::A] != 0) && (((sint)_RequestedProps[CHarvestSource::Q]) < (sint)harvestSource->forageSite()->deposit()->minQuality()))
	{
	...
	if ((_RequestedProps[CHarvestSource::A] == 0) && (_RequestedProps[CHarvestSource::ReduceDmg] != 0))
	{
		if (_NbExtractions != 0)
			return false;
	}
...
void CFgExtractionPhrase::applyExtraction(CCharacter *player, float successFactor)
{
	...
	if (!isCareAction())
	{
		...
		if (player->forageProgress()->amount() > previousAmount)
		{
			...
			behav.DeltaHP = (sint16)(player->forageProgress()->amount() | (player->forageProgress()->quality() << 8));
			behav.ForageExtraction.Level = calcLevelFromSkillValue(_SkillValue);
			PHRASE_UTILITIES::sendUpdateBehaviour(_ActorRowId, behav);
		}
	}
