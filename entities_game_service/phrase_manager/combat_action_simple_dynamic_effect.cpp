...
//					apply()
//--------------------------------------------------------------
void CCombatActionDynamicEffect::apply(CCombatPhrase *phrase)
{
#if !FINAL_VERSION
	nlassert(phrase);
#endif
	H_AUTO(CCombatActionDynamicEffect_apply);

	_CombatPhrase = phrase;

	_EffectValue = (sint32)getApplyValue(phrase->weaponSabrinaValue());
...
void CCombatActionDynamicEffect::applyOnEntity(CEntityBase *entity, float successFactor)
{
	...
	CSimpleEffect *effect = new CSimpleEffect(_ActorRowId, entity->getEntityRowId(), _Family, _EffectValue, endDate, uint8(_EffectValue));
	if (!effect)
	{
		nlwarning("COMBAT : <CCombatActionDynamicEffect::apply> Failed to allocate new CSimpleEffect object !");
		return;
	}
