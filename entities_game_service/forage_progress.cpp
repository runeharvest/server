...
void CForageProgress::reportXP(CCharacter *extractor, const CHarvestSource *source)
{
	...
	float xpBonusFactor = 0.9f - ForageProspectionXPBonusRatio.get() + ((((float)(source->getStatQuality())) - 5.0f) / (75.0f * 5.0f)); // basic gives -10%, supreme gives +10% bonus (5 -> 0.0, 80 -> 0.2)
