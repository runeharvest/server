			...
			if ((*it).second == user->getEntityRowId())
			{
				...
				else
				{
					...
					if (oldZone->getPVPZoneType() == PVP_ZONE_TYPE::VersusZone && zone->getPVPZoneType() == PVP_ZONE_TYPE::VersusZone)
					{
						...
						if ((oldVersusZone->getClan(1) == newVersusZone->getClan(1) && oldVersusZone->getClan(2) == newVersusZone->getClan(2))
						    || (oldVersusZone->getClan(1) == newVersusZone->getClan(2) && oldVersusZone->getClan(2) == newVersusZone->getClan(1)))
						{
							_UsersLeavingPVPZone.erase(it);
