...
bool CPVPFactionRewardManager::isAttackable(CCharacter *actor, CEntityBase *target)
{
	...
	if (region)
	{
		...
		if (pTotemBase)
		{
			...
			if (CPVPManager2::getInstance()->factionWarOccurs(allegiance.first, pTotemBase->getOwnerFaction()) || CPVPManager2::getInstance()->factionWarOccurs(allegiance.second, pTotemBase->getOwnerFaction()))
				return true;
		}
	}
