...
CTotemBase::TRewardCategory CTotemBase::_GetRewardCatergory(CCharacter *user) const
{
	...
	if (CPVPManager2::getInstance()->factionWarOccurs(allegiance.first, _OwnerFaction) || CPVPManager2::getInstance()->factionWarOccurs(allegiance.second, _OwnerFaction))
...
void CTotemBase::destroyTotem()
{
	...
	if (_BotObject)
	{
		...
		_BotObject->getScores()._PhysicalScores[SCORES::hit_points].Current = _BotObject->getScores()._PhysicalScores[SCORES::hit_points].Max;
