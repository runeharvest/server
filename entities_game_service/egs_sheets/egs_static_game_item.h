...
public:
	...
	static RM_GROUP::TRMGroup stringToRmGroup(const std::string &groupName)
	{
		CRmGroupByName::const_iterator ig = _RMGroupsByName.find(groupName);
		if (ig != _RMGroupsByName.end()) return (*ig).second;
		else return RM_GROUP::Unknown;
	}
...
struct CSpeedUpDPLossData : public IItemServiceData
{
	...
	IItemServiceData *clone()
	{
		CSpeedUpDPLossData *ptr = new CSpeedUpDPLossData();
		*ptr = *this;
		return ptr;
	}
