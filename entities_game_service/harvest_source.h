...
public:
	...
	{
		S,
		NoDrop = S, // Source speed (in steps per tick (<1)). Not droppable.
		A = 1, // Source aperture (RM quantity delivered per step). Request 0 or "care".
		Q = 2,
		ReduceDmg = Q, // Average quality of the delivered RM
		NbPosRTProps,
	...
	void setBonusExtraExtractionTime(NLMISC::TGameCycle t)
	{
		_ExtraExtractionTime += t - _IncludedBonusExtractionTime;
		_IncludedBonusExtractionTime = t;
	}
	...
	void resetSource(const CHarvestSource &ini)
	{
		_NbExtractions = 0;
		initFrom(ini);
	}
...
protected:
	...
	{
		_LifeTime = ini._LifeTime;
		_ExtractionTime = ini._ExtractionTime;
		_ExtraExtractionTime = ini._ExtraExtractionTime;
		_S = ini._S;
		_A = ini._A;
		_Q = ini._Q;
		_D = ini._D;
		_E = ini._E; /*_C=ini._C;*/ /*_DistanceVisibility=ini._DistanceVisibility; _StealthVisibility=ini._StealthVisibility;*/
	}
	...
	{
		if (_NbEventTriggered < 255) ++_NbEventTriggered;
	}
