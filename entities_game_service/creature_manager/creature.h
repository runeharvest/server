...
public:
	...
	///@name Partial reset
	//@{
	virtual void resetFaction()
	{
		if (_Sheet) _Faction = _Sheet->getFaction();
	}
	virtual void resetFameByKillValid()
	{
		if (_Sheet) _FameByKillValid = _Sheet->getFameByKillValid();
	}
	virtual void resetFameByKill()
	{
		if (_Sheet) _FameByKill = _Sheet->getFameByKill();
	}
	//@}
...
public:
	...
	void resetFaction()
	{
		if (_Form) _Faction = _Form->getFaction();
	}
	void resetFameByKillValid()
	{
		if (_Form) _FameByKillValid = _Form->getFameByKillValid();
	}
	void resetFameByKill()
	{
		if (_Form) _FameByKill = _Form->getFameByKill();
	}
	...
	NLMISC::CSheetId getLootTable(uint i)
	{
		if (i < _LootTables.size()) return _LootTables[i];
		else return NLMISC::CSheetId::Unknown;
	};
