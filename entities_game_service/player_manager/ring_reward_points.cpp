...
CRingRewardPointsImpl::CRingRewardPointsImpl(CCharacter *theCharacter)
...
void CRingRewardPointsImpl::tryToValidateTimeSlice()
{
	...
	if (RingRPMode == 2)
	{
		...
		// setup the number of points to be gained
		uint32 pointsGainedThisTime = (numActivePlayers == 1) ? 25 : (numActivePlayers == 2) ? 50
		...
		pointsGainedThisTime = (numRRPGains == 0) ? (uint32)(pointsGainedThisTime * 0.75) : (numRRPGains == 1) ? (uint32)(pointsGainedThisTime * 1.0)
...
void CRingRewardPointsImpl::addXp(R2::TSessionLevel xpLevel, uint32 xpValue)
{
	...
	if (RingRPMode == 1)
	{
		...
		_LastTimeSliceStartTime = (deltaTime < (3 * minTimeSliceDuration) / 2) ? _LastTimeSliceStartTime + minTimeSliceDuration : (deltaTime < 2 * minTimeSliceDuration) ? _LastTimeSliceStartTime + (timeNow - _LastTimeSliceStartTime - minTimeSliceDuration) * 2 / minTimeSliceDuration
		                                                                                                                                                                 :
		                                                                                                                                                                 /* default */ timeNow;
...
#pragma message(NL_LOC_WRN "Implements mission step validation")
		...
		for (R2::TSessionLevel rewardLevel = R2::TSessionLevel::sl_a; rewardLevel.isValid(); rewardLevel = (R2::TSessionLevel::TValues)(rewardLevel.getValue() + 1))
		{
			...
			if (playerRewardPoints < RingRPPointsPerReward)
			{
				if (bestRRP >= playerRewardPoints)
				{
					bestLevel = rewardLevel;
					bestRRP = playerRewardPoints;
				}
				continue;
			}
...
#define PERSISTENT_DATA                                                                                                                                                                                    \
	LPROP_MAP2(RingRewardPoints, NLMISC::CSString, uint32, ARRAY_LOGIC((uint32)R2::TSessionLevel::nb_enum_items), (NLMISC::CSString("Level") + char('A' + i)), _CumulatedRRP[i], if (key.size() == 6) { uint32 idx=(key[5]-'A'); if (idx <R2::TSessionLevel::nb_enum_items) _CumulatedRRP[idx]=val; }) \
	LPROP_MAP2(RingRewardPointsHistory, NLMISC::CSString, uint32, ARRAY_LOGIC((uint32)R2::TSessionLevel::nb_enum_items), (NLMISC::CSString("Level") + char('A' + i)), _HistoricRRP[i], if (key.size() == 6) { uint32 idx=(key[5]-'A'); if (idx <R2::TSessionLevel::nb_enum_items) _HistoricRRP[idx]=val; })
