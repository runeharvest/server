...
public:
	...
	/// ctor
	CInventoryProxy(CCharacter *owner, const CInventoryPtr &inv)
	    : _Owner(owner)
	    , _Inventory(inv)
	{
	}
...
#define PERSISTENT_DATA                                                                                                                                                                          \
	...
	LPROP_MAP2(FactionPoints, string, uint32,                                                                                                                                                    \
	           ...
	           _FactionPoint[i],                                                                                                                                                                 \
	           PVP_CLAN::TPVPClan k = PVP_CLAN::fromString(key);                                                                                                                                 \
	           if ((k >= PVP_CLAN::BeginClans) && (k <= PVP_CLAN::EndClans)) _FactionPoint[k - PVP_CLAN::BeginClans] = val)                                                                      \
                                                                                                                                                                                                 \
	...
	LPROP_MAP2(SkillPoints, string, double,                                                                                                                                                      \
	           ...
	           _SpType[i],                                                                                                                                                                       \
	           CSPType::TSPType k = CSPType::fromString(key);                                                                                                                                    \
	           if (k != CSPType::Unknown) _SpType[k] = val)                                                                                                                                      \
                                                                                                                                                                                                 \
	LPROP_MAP2(SpentSkillPoints, string, uint32,                                                                                                                                                 \
	           ...
	           _SpentSpType[i],                                                                                                                                                                  \
	           CSPType::TSPType k = CSPType::fromString(key);                                                                                                                                    \
	           if (k != CSPType::Unknown) _SpentSpType[k] = val)                                                                                                                                 \
                                                                                                                                                                                                 \
	LPROP_MAP2(ScorePermanentModifiers, string, sint32,                                                                                                                                          \
	           ...
	           SCORES::TScores k = SCORES::toScore(key);                                                                                                                                         \
	           if (k != SCORES::unknown) _ScorePermanentModifiers[k] = val)                                                                                                                      \
                                                                                                                                                                                                 \
	LPROP_MAP2(StartingCharacteristicValues, string, uint8,                                                                                                                                      \
	           ...
	           if (k != CHARACTERISTICS::Unknown) _StartingCharacteristicValues[k] = val)                                                                                                        \
                                                                                                                                                                                                 \
	...
	LSTRUCT_MAP2(Inventory, string,                                                                                                                                                              \
	    for (uint32 i = 0; i < INVENTORIES::NUM_INVENTORY; ++i) if (i != INVENTORIES::handling && i != INVENTORIES::equipment) if (_Inventory[i] != NULL && _Inventory[i]->getSlotCount() != 0), \
...
#define PERSISTENT_DATA                                                                    \
	...
	PROP2(StableAlias, TAIAlias, stableAlias, if (val != CAIAliasTranslator::Invalid) {\
...
#define PERSISTENT_DATA   \
	FLAG0(CLEAR, clear()) \
	LSTRUCT_MAP2(Phrases, uint32, VECT_LOGIC(Phrases) if (Phrases[i] != NULL), i, Phrases[i]->store(pdr), if (key < Phrases.size()) { \
			if (Phrases[key]==NULL) \
...
#define PERSISTENT_DATA   \
	FLAG0(CLEAR, clear()) \
	LPROP_MAP2(_PhysicalCharacteristics, string, sint32, for (uint32 i = 0; i < CHARACTERISTICS::NUM_CHARACTERISTICS; ++i), CHARACTERISTICS::toString((CHARACTERISTICS::TCharacteristics)i), _PhysicalCharacteristics[i].Base, CHARACTERISTICS::TCharacteristics k = CHARACTERISTICS::toCharacteristic(key); if (k != CHARACTERISTICS::Unknown) { _PhysicalCharacteristics[k].Base=val; _PhysicalCharacteristics[k].Current=val; _PhysicalCharacteristics[k].Max=val; displayInfo(string()+"Setting "+key+": "+toString(val)); } else displayWarning(string() + "Failed to set " + key + ": " + toString(val)))
...
#define PERSISTENT_DATA                                                        \
	...
	LSTRUCT_MAP2(PhysicalScores, string,                                       \
	             ...
	             SCORES::TScores k = SCORES::toScore(key);                     \
	             if (k != SCORES::unknown) _PhysicalScores[k].apply(pdr))
...
#define PERSISTENT_DATA                                                                     \
	...
	LSTRUCT_MAP2(Skills, string,                                                            \
	             ...
	             _Skills[i].store(pdr),                                                     \
	             SKILLS::ESkills k = SKILLS::toSkill(key);                                  \
	             if (k != SKILLS::unknown) _Skills[k].apply(pdr))
...
public:
	...
	/// ctor
	COldPlayerRoomInventoryLoader(const CInventoryPtr &inv)
	    : _Inventory(inv)
	{
	}
