...
public:
	CISPFeedAnimal(ITEM_SERVICE_TYPE::TItemServiceType serviceType, uint32 animalClientPos)
	    : IItemServiceProvider(serviceType)
	    , _AnimalClientPos(animalClientPos)
	{
...
public:
	CISPFeedAllAnimals()
	    : IItemServiceProvider(ITEM_SERVICE_TYPE::StableFeedAllAnimals)
	{
	}
...
public:
	CISPSpeedUpDPLoss()
	    : IItemServiceProvider(ITEM_SERVICE_TYPE::SpeedUpDPLoss, new CPersistent)
	{
	}
