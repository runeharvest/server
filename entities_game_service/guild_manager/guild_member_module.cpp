...
CGuildMemberModule::CGuildMemberModule(CGuildCharProxy &proxy, CGuildMember *guildMember)
    : IModule(&(proxy.getModuleParent()), guildMember)
    , _GuildMemberCore(guildMember)
{
	nlassert(guildMember);
	...
	if (invitorPlayer)
	{
		...
		if (targetEntity)
		{
			if (targetEntity->getOutpostAlias() != 0)
			{
				...
				if (outpost)
				{
					if ((outpost->getOwnerGuild() == invitorPlayer->getGuildId() && targetEntity->getOutpostSide() == OUTPOSTENUMS::OutpostAttacker) || (outpost->getAttackerGuild() == invitorPlayer->getGuildId() && targetEntity->getOutpostSide() == OUTPOSTENUMS::OutpostOwner))
					{
