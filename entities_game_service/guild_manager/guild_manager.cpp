...
void CGuildManager::characterDeleted(CCharacter &user)
{
	...
	if (userMember->getGrade() == EGSPD::CGuildGrade::Leader)
	{
		     ...
		     ++it)
		{
			...
			if (successor == NULL || member->getGrade() < successor->getGrade() || (member->getGrade() == successor->getGrade() && member->getEnterTime() < successor->getEnterTime()))
			{
				successor = member;
			}
		}
...
void CGuildManager::callback(const CFileDescription &fileDescription, NLMISC::IStream &dataStream)
{
	...
	if (file.size() == len && (file.find("bin") == len - 3)
	    || file.find("txt") == len - 3)
	{
