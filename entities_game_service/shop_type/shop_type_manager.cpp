					...
					if (itemForm->Origin != ITEM_ORIGIN::UNKNOWN)
					{
						for (uint quality = 0; quality < min((uint)NUM_QUALITY, (uint)(_QualityEnd - _QualityStart - 1)); ++quality)
						{
							...
							for (uint level = 0; level < maxLevel; ++level)
							{
								...
								if (itemTypeEnum == ITEM_TYPE::HAIR_FEMALE || itemTypeEnum == ITEM_TYPE::HAIRCOLOR_FEMALE || itemTypeEnum == ITEM_TYPE::TATOO_FEMALE || itemTypeEnum == ITEM_TYPE::HAIR_MALE || itemTypeEnum == ITEM_TYPE::HAIRCOLOR_MALE || itemTypeEnum == ITEM_TYPE::TATOO_MALE || itemTypeEnum == ITEM_TYPE::MEKTOUB_MOUNT_TICKET || itemTypeEnum == ITEM_TYPE::MEKTOUB_PACKER_TICKET || itemTypeEnum == ITEM_TYPE::FOOD || itemTypeEnum == ITEM_TYPE::SERVICE_STABLE)
									break;
							}
							if (itemTypeEnum == ITEM_TYPE::HAIR_FEMALE || itemTypeEnum == ITEM_TYPE::HAIRCOLOR_FEMALE || itemTypeEnum == ITEM_TYPE::TATOO_FEMALE || itemTypeEnum == ITEM_TYPE::HAIR_MALE || itemTypeEnum == ITEM_TYPE::HAIRCOLOR_MALE || itemTypeEnum == ITEM_TYPE::TATOO_MALE || itemTypeEnum == ITEM_TYPE::MEKTOUB_MOUNT_TICKET || itemTypeEnum == ITEM_TYPE::MEKTOUB_PACKER_TICKET || itemTypeEnum == ITEM_TYPE::FOOD || itemTypeEnum == ITEM_TYPE::SERVICE_STABLE)
								break;
