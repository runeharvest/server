...
CGameItemPtr CStaticItems::buildOnetItem(CSheetId sheet)
{
	...
	if (staticItem != 0)
	{
		if (staticItem->Family != ITEMFAMILY::AMMO && staticItem->Family != ITEMFAMILY::ARMOR && staticItem->Family != ITEMFAMILY::JEWELRY && staticItem->Family != ITEMFAMILY::MELEE_WEAPON && staticItem->Family != ITEMFAMILY::RANGE_WEAPON && staticItem->Family != ITEMFAMILY::SHIELD)
			return 0;
		...
		if (staticItem->Family == ITEMFAMILY::MELEE_WEAPON || staticItem->Family == ITEMFAMILY::RANGE_WEAPON || staticItem->Family == ITEMFAMILY::AMMO || staticItem->Family == ITEMFAMILY::ARMOR || staticItem->Family == ITEMFAMILY::SHIELD || staticItem->Family == ITEMFAMILY::JEWELRY)
		{
			CFaberPhrase phrase;
			vector<CSheetId> Rm;
