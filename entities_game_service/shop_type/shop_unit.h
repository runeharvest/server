...
public:
	...
	CShopUnitDynamic()
	{
		_ShopUnitType = IShopUnit::DynamicShop;
		_ShopContent.resize(CONTINENT::NB_CONTINENTS);
		_CycleContentChange = 0;
	}
	...
	// return trade list reference corresponding to item shop content
	const TTradeList &getShopContent(CONTINENT::TContinent continent) const
	{
		nlassert(((uint32)continent) < _ShopContent.size());
		return _ShopContent[continent];
	}
