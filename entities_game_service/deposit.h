...
public:
	...
	CRecentForageSite(CDeposit *dep, const NLMISC::CVector &pos)
	    : _Deposit(dep)
	    , _Pos(pos)
	    , _TimeToLive(ForageSiteNbUpdatesToLive.get())
	    , _LowScopeStock(ForageSiteStock.get())
	    , _NbActiveSources(0)
	{
	}

	/// Add a source
	bool addActiveSource()
	{
		if (_NbActiveSources == MaxNbActiveSources) return false;
		++_NbActiveSources;
		return true;
	}
...
public:
	...
	CDeposit()
	    ...
	    , _CanProspect(false)
	    , _Enabled(false)
	    , _CurrentNbAutoSpawnedSources(0)
	    , _AllowDepletionRisk(true)
	    , _Alias(0)
	{
	}
	...
	float decKamiAnger(float delta)
	{
		if (_KamiAnger != -1.0f)
		{
			_KamiAnger -= delta;
			if (_KamiAnger < 0) _KamiAnger = 0;
		}
		return _KamiAnger;
	}
	...
	float consumeQuantity(float requested)
	{
		if (_QuantityConstraintsPt) return _QuantityConstraintsPt->consumeQuantity(requested);
