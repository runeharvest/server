	...
	for (uint i = 0; i < getNumChildren(); ++i)
	{
		...
		if (getChild(child, i) && child)
		{
			...
			if (idx != InvalidSpawnZoneId)
			{
				...
				else if (spawn->getType() == RESPAWN_POINT::KAMI || spawn->getType() == RESPAWN_POINT::KARAVAN || spawn->getType() == RESPAWN_POINT::NEWBIELAND || spawn->getType() == RESPAWN_POINT::RESPAWNABLE)
				{
...
bool CZoneManager::parseTpSpawnZones(const NLLIGO::IPrimitive *prim)
{
	...
	if (prim->getPropertyByName("class", value))
	{
		if (value == "teleport_spawn_zone")
		{
			...
			if (zone.getType() == RESPAWN_POINT::KAMI || zone.getType() == RESPAWN_POINT::KARAVAN || zone.getType() == RESPAWN_POINT::NEWBIELAND || zone.getType() == RESPAWN_POINT::RESPAWNABLE)
			{
				bool found = false;
				uint j;
