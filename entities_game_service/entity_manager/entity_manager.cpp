...
void CQueryEgsImp::callback(const std::string &name, NLNET::TServiceId id)
{
	...
	// Score  / MaxScore
	case CQueryEgs::RatioHp: {
		sint32 val = character->getMaxScore(SCORES::hit_points);
		ret = NLMISC::toString("%d", val ? 100 * character->getCurrentScore(SCORES::hit_points) / val : 0);
	}
	break;
	case CQueryEgs::RatioSap: {
		sint32 val = character->getMaxScore(SCORES::sap);
		ret = NLMISC::toString("%d", val ? 100 * character->getCurrentScore(SCORES::sap) / val : 0);
	}
	break;
	case CQueryEgs::RatioStamina: {
		sint32 val = character->getMaxScore(SCORES::stamina);
		ret = NLMISC::toString("%d", val ? 100 * character->getCurrentScore(SCORES::stamina) / val : 0);
	}
	break;
	case CQueryEgs::RatioFocus: {
		sint32 val = character->getMaxScore(SCORES::focus);
		ret = NLMISC::toString("%d", val ? 100 * character->getCurrentScore(SCORES::focus) / val : 0);
	}
	break;
	...
	case CQueryEgs::KnowBrick: { // verify if a brick identified by its sheet id is known by the player
