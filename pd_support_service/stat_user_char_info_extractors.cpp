...
{
	NLMISC::CSString dateStr = buildDateString(c->_LastConnectedTime);
	if (dateStr.empty())
		return;

	job->charTblSetEntry("last_date", dateStr);
	job->freqTblAddEntry("last_date", dateStr);
}

INFO_EXTRACTOR(FirstActivityDate, "The first time that the player connected", "start_date")
{
	NLMISC::CSString dateStr = buildDateString(c->_FirstConnectedTime);
	if (dateStr.empty())
		return;

	job->charTblSetEntry("start_date", dateStr);
	job->freqTblAddEntry("start_date", dateStr);
}
...
INFO_EXTRACTOR(Position, "x y position and place", "x,y,place")
{
	// setup x and y coordinates
	sint32 x, y;
	if (c->NormalPositions._Vec.empty())
	{
		x = c->EntityBase._EntityPosition.X / 1000;
		y = c->EntityBase._EntityPosition.Y / 1000;
	}
	else
	{
		x = c->NormalPositions._Vec.back().PosState.X / 1000;
		y = c->NormalPositions._Vec.back().PosState.Y / 1000;
	}

	// xy stats
	job->charTblSetEntry("x", NLMISC::toString(x));
	job->charTblSetEntry("y", NLMISC::toString(y));

	// setup the place name
	NLMISC::CSString placeName;
	if (x >= 8160 && x <= 11360 && y >= -12320 && y <= -10240)
	{
		if (abs(x - 10317) + abs(y + 11720) < 30)
		{
			placeName = "ros-rc-chong";
		}
		else if (abs(x - 10423) + abs(y + 11675) < 30)
		{
			placeName = "ros-rc-kami";
		}
		else if (abs(x - 10402) + abs(y + 11864) < 30)
		{
			placeName = "ros-rc-kara";
		}
		else
		{
			placeName = "ros";
	...
	// add '(dead)' to the place name if the player is dead
	if (c->EntityBase._PhysScores.PhysicalScores.find("HitPoints")->second.Current <= 0)
		placeName += "(dead)";

	// place stats
	job->charTblSetEntry("place", placeName);
	job->freqTblAddEntry("place", placeName);
}
...
INFO_EXTRACTOR(Sessions, "home session id and current session id", "home,session")
{
	...
	// lookup the player's home session name
	uint32 homeSessionId = c->NormalPositions._Vec[0].SessionId;
	NLMISC::CSString homeShardName = lookupSessionName(homeSessionId);

	job->charTblSetEntry("home", homeShardName);
	job->freqTblAddEntry("home_shard", homeShardName);
	...
	// lookup the player's current session name
	uint32 curSessionId = c->NormalPositions._Vec.back().SessionId;
	NLMISC::CSString curSessionName = lookupSessionName(curSessionId);

	job->charTblSetEntry("session", curSessionName);
}
...
INFO_EXTRACTOR(ActiveMissions, "frequency stats regarding active missions", "act_missions")
{
	CStatsScanMissions::TMissions::const_iterator it = c->_Missions.Missions.begin();
	CStatsScanMissions::TMissions::const_iterator itEnd = c->_Missions.Missions.end();
	for (; it != itEnd; ++it)
	{
		job->freqTblAddEntry("act_mission_id", NLMISC::toString("A:%u:%u", it->first >> 20, it->first << 12 >> 12));
	}
	job->charTblSetEntry("act_missions", NLMISC::toString(c->_Missions.Missions.size()));
}

INFO_EXTRACTOR(MissionHistory, "frequency stats regarding completed missions", "hist_missions")
{

	std::map<TAIAlias, CStatsScanMissionHistory>::const_iterator it = c->_MissionHistories.begin();
	std::map<TAIAlias, CStatsScanMissionHistory>::const_iterator itEnd = c->_MissionHistories.end();
	for (; it != itEnd; ++it)
	{
		job->freqTblAddEntry("hist_mission_id", NLMISC::toString("A:%u:%u", it->first >> 20, it->first << 12 >> 12));
	}
	job->charTblSetEntry("hist_missions", NLMISC::toString(c->_MissionHistories.size()));
}
...
INFO_EXTRACTOR(HighestSkills, "Highest skills in each main branch", "skill_best,skill_sf,skill_sm,skill_sc,skill_sh")
{
	...
	for (TSkills::const_iterator it = skills.begin(); it != skills.end(); ++it)
	{
		...
		{
		case 'F':
			if (bestF <= (*it).second.Current) bestF = (*it).second.Current;
			bestFName = (*it).first;
			break;
		case 'M':
			if (bestM <= (*it).second.Current) bestM = (*it).second.Current;
			bestMName = (*it).first;
			break;
		case 'C':
			if (bestC <= (*it).second.Current) bestC = (*it).second.Current;
			bestCName = (*it).first;
			break;
		case 'H':
			if (bestH <= (*it).second.Current) bestH = (*it).second.Current;
			bestHName = (*it).first;
			break;
		}
		job->freqTblAddEntry("all_skills", NLMISC::toString("%s,%d", (*it).first.c_str(), (*it).second.Current));
	}
	if (bestF >= bestM && bestF >= bestH && bestF >= bestC)
	{
		best = bestF;
		bestName = bestFName;
	}
	else if (bestM >= bestF && bestM >= bestH && bestM >= bestC)
	{
		best = bestM;
		bestName = bestMName;
	}
	else if (bestH >= bestF && bestH >= bestM && bestH >= bestC)
	{
		best = bestH;
		bestName = bestHName;
	}
