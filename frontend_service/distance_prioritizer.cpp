...
#define arbitrateDiscreetProperty(entry, name)                                             \
	GET_VP_NODE(name)->BranchHasPayload = entityIsWithinDistanceThreshold(PROPERTY_##name) \
...
#define arbitrateDiscreetPropertyWithoutThreshold(entry, name) \
	GET_VP_NODE(name)->BranchHasPayload = discreetPropertyHasChanged(entry.Properties[PROPERTY_##name], sentity->VP_##name, PROPERTY_##name, (TYPE_##name *)NULL)
...
#define arbitrateDiscreetPropertyWithoutThreshold(entry, name) \
	GET_VP_NODE(name)->BranchHasPayload = discreetPropertyHasChanged(entry.Properties[PROPERTY_##name], PROPERTY_##name, (TYPE_##name *)NULL)
...
#define arbitrateTargetList(entry, name) \
	GET_VP_NODE(name)->BranchHasPayload = targetListHasChanged(entry.Properties[PROPERTY_##name], PROPERTY_##name, (TYPE_##name *)NULL)
		...
		else
		{
			...
			if (_VisualPropertyTreeRoot->BranchHasPayload)
			{
				...
				if (GET_VP_NODE(POSITION)->BranchHasPayload)
				{
					...
					if ((!(TVPNodeServer::PrioContext.ZCache & 0x1)) || (GET_VP_NODE(POSITION)->BranchHasPayload && GET_VP_NODE(MODE)->BranchHasPayload))
					{
...
#endif
    void
    CDistancePrioritizer::arbitrateDiscreetBehaviourProperty(const CPropertyHistory::CEntityEntry &entry, CEntity *sentity)
{
#ifdef STORE_MIRROR_VP_IN_CLASS
...
void CDistancePrioritizer::arbitrateNPCDiscreetProperties(const CPropertyHistory::CEntityEntry &entry)
{
	...
	GET_VP_NODE(CONTEXTUAL)->BranchHasPayload = TVPNodeServer::PrioContext.DistanceCE < THRESHOLD_CONTEXTUAL_NPC
