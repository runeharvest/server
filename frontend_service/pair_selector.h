...
public:
	// Constructor
	TPairCE(TClientId c, CLFECOMMON::TCLEntityId e)
	    : ClientId(c)
	    , CeId(e)
	{
	}
...
protected:
	...
	TPairRange &lastRange()
	{
		nlassert(!_Ranges.empty());
		return _Ranges.back();
	}

	/// Easy access to the last range for pseudosort (const version)
	const TPairRange &lastRange() const
	{
		nlassert(!_Ranges.empty());
		return _Ranges.back();
	}
