...
{
	TComparePairsByDistance(CVisionArray *va, TClientId clientId)
	    : _VisionArray(va)
	    , _ClientId(clientId)
	{
	}
...
const char *associationStateToString(uint8 as)
{
	...
	case TPairState::AwaitingDisAck:
		return "Disass";
		break;
...
void CClientHost::displayClientProperties(bool full, bool allProps, bool sortByDistance, NLMISC::CLog *log) const
{
	...
	log->displayNL("Latest activity %u ms ago, state %s", (uint32)(CTime::getLocalTime() - _ReceiveTime),
	    ConnectionState == Synchronize ? "SYNC" : ConnectionState == Connected ? "CONN"
	        : ConnectionState == Probe                                         ? "PROBE"
	        : ConnectionState == ForceSynchronize                              ? "FORCE_SYNC"
	                                                                           : "DISC");
#ifdef HALF_FREQUENCY_SENDING_TO_CLIENT
	sint freq = 5;
#else
