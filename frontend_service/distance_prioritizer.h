...
#else
#define LOG_WHAT_IS_SENT                                                                                    \
	if ((verbosePropertiesSent != 0) && (verbosePropertiesSent != TVPNodeServer::PrioContext.ClientId)) { } \
	else nldebug
#endif
...
public:
	...
	{
		CSortedFlatVPTreeItem()
		    : Node(NULL)
		    , APayLoad(&FalseBoolPayLoad)
		    , BPayLoad(&FalseBoolPayLoad)
		{
		}
...
#define SET_FILLFUNC(name)   \
	INIT_FLAT_VP_TREE(name); \
	((TVPNodeServer *)(discreetRoot->get##name##node()))->FillFunc = fill##name
#define SET_FILLFUNC2(nameProp, nameFunc) \
	INIT_FLAT_VP_TREE(nameProp);          \
	((TVPNodeServer *)(discreetRoot->get##nameProp##node()))->FillFunc = fill##nameFunc
