...
class CCharacterSync : public CEmptyModuleCommBehav<CEmptyModuleServiceBehav<CEmptySocketBehav<CModuleBase>>>,
	...
	void syncUserChars(NLNET::IModuleProxy *sender, uint32 userId, const std::vector<TCharInfo> &charInfos)
	{
		...
		for (uint i = 0; first != last; ++first, ++i)
		{
			...
			if (currentSessionId != 0)
			{
				...
				if (session != NULL)
				{
					isOwnerOfResumableSession = ((session->getOwnerId() == first->first) && (session->getSessionType() == RSMGR::TSessionType::st_anim) && (session->getState() == RSMGR::TSessionState::ss_open));
