...
class CRingSessionManager : public CEmptyModuleServiceBehav<CEmptyModuleCommBehav<CEmptySocketBehav<CModuleBase>>>,
                            ...
                            public ICharacterSyncCb
{
	...
	{
	public:
		COptionalSecurityCode()
		    : _Enabled(false)
		{
		}
		void setSecurityCode(const CSecurityCode &sc)
		{
			_SecurityCode = sc;
			_Enabled = true;
		}
		const CSecurityCode *getSecurityCode() const
		{
			if (_Enabled) return &_SecurityCode;
			else return NULL;
		} // return NULL if not included
...
public:
	    ...
	    uint32 rateDirection)
	{
		...
		BOMB_IF(!character->loadSessionParticipants(_RingDb, __FILE__, __LINE__),
		        "Failed to load session participation for char " << charId, invokeResult(from, charId >> 4, 3, "Failed to load participation");
		        return);
		uint i = 0;
		for (i = 0; i < character->getSessionParticipants().size(); ++i)
		{
		...
		BOMB_IF(i == character->getSessionParticipants().size(),
		        "No participation found for char " << charId << " in session " << sessionId.asInt(), invokeResult(from, charId >> 4, 3, "No participation found in session");
		        return);
