...
public:
	...
	explicit CObjectIndex(bool validateChecksum = false)
	    : _Row(INVALID_ROW_INDEX)
	    , _Table(INVALID_TABLE_INDEX)
	    , _Checksum((TIndexChecksum)~VALID_INDEX_CHECKSUM)
	{
		if (validateChecksum)
			validate();
		else
	...
	/// Constructor
	CObjectIndex(TTableIndex table, TRowIndex row)
	    : _Row(row)
	    , _Table(table)
	{
		validate();
	...
	/// Copy
	CObjectIndex &operator=(const CObjectIndex &index)
	{
		_FullIndex = index._FullIndex;
		return *this;
	}
...
public:
	...
	private:
		...
		CAccessor(TListMap::iterator it, const CColumnIndex &index)
		    : _It(it)
		    , _Valid(true)
		    , _Debug(index)
		{
