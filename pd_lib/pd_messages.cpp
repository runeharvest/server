...
bool CDbMessage::contains(const CDBDescriptionParser &description, const NLMISC::CEntityId &id)
{
	...
	try
	{
		...
		case SetParent:
			if ((objectEntityIdPresent() && compareEId(getObjectId(), id)) || (parentsEntityIdPresent() && (compareEId(getNewParentId(), id) || compareEId(getPreviousParentId(), id))))
				return true;
			break;
    ...
    float *progress)
{
	...
	{
		char buffer[64];
		if (NLMISC::CFile::getExtension(files[i]) != "pd_log" || sscanf(NLMISC::CFile::getFilenameWithoutExtension(files[i]).c_str(), "%28s", buffer) != 1)
		{
