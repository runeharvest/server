...
{
	TMessageCarrier(bool inputStream)
	    : Msg(inputStream)
	{
	}
...
{
	TClientServiceInfoOwnedRange(CDataSetMS *dataset = NULL)
	    : DataSet(dataset)
...
public:
	...
	/// Assignment operator
	CMTRTag &operator=(const CMTRTag &other)
	{
		_RawTag = other._RawTag;
		return *this;
	}
...
public:
	/// Constructor
	CMirrorPropValueItemMS(CMirrorPropValueListMS<T> *parentlist, T *pt)
...
public:
	...
	CMirrorPropValueItemMS(CMirrorPropValueListMS<NLMISC::CEntityId> *parentlist, uint64 *pt)
	    : _ParentList(parentlist)
	    , _Pt(pt)
	{
	}
...
struct _CMirrorPropValueListIteratorMS
{
	...
	_CMirrorPropValueListIteratorMS &operator++()
	{
		_Index = _ParentList->_Container[_Index].Next;
		return *this;
	}
	_CMirrorPropValueListIteratorMS operator++(int)
	{
		_CMirrorPropValueListIteratorMS tmp = *this;
		_Index = _ParentList->_Container[_Index].Next;
		return tmp;
	}
	bool operator==(const _CMirrorPropValueListIteratorMS &other) const { return (_Index == other._Index) && (_ParentList == other._ParentList); }
	bool operator!=(const _CMirrorPropValueListIteratorMS &other) const { return (_Index != other._Index) || (_ParentList != other._ParentList); }
	_CMirrorPropValueListIteratorMS &operator=(const _CMirrorPropValueListIteratorMS &other)
	{
	...
	_CMirrorPropValueListIteratorMS()
	    : _ParentList(NULL)
...
#else
	template <class U>
	friend class _CMirrorPropValueListIteratorMS; // GCC3
#endif
...
enum TGroupTrackerState
{
	Normal,
	IsGroupTracker,
	PointingToGroupTracker
};
...
public:
	...
	/// Constructor
	CChangeTrackerMS(NLNET::TServiceId destServiceId, bool local, bool readOnly = false, bool writeOnly = false, TGroupTrackerState state = Normal)
	    : CChangeTrackerBase()
	    , _DestServiceId(destServiceId)
	    , _Local(local)
	    , _GroupTrackerState(state)
	    , _NbInterestedServices(0)
	    , _StoredNbChanges(0)
	    , _BlockingInfo(NULL)
	    , _LinkToDeltaBuffer(NULL)
	{
		resetFlags(readOnly, writeOnly);
		if (!local) allowSendingToRemoteMS(false);
	}
#else
	/// Constructor
	CChangeTrackerMS(NLNET::TServiceId destServiceId, bool local, bool readOnly = false, bool writeOnly = false, TGroupTrackerState state = Normal)
	...
	uint nbATEAcksExpectedBeforeUnblocking() const
	{
		nlassert(_BlockingInfo);
		return _BlockingInfo->nbAcksExpected();
	}
	...
	const std::vector<NLNET::TServiceId8> &remoteClientServicesToSync()
	{
		nlassert(_BlockingInfo);
		return _BlockingInfo->remoteClientServicesToSync();
	}
...
public:
	/// Constructor
	CDataSetMS()
	    : _MaxOutBandwidth(1024 * 1024)
	    , _NbLocalWriterProps(0)
	    , _BindingCountsToSet(NULL) /*, NbOnlineEntities(0)*/
	{
	}
	...
	/// Change a weight
	void changeWeightOfProperty(TPropertyIndex propIndex, sint32 weight)
	{
		_Weights[propIndex] = weight;
		calculateChangesBudgets();
	}
	...
	void setMaxOutBandwidth(uint32 mob)
	{
		_MaxOutBandwidth = mob;
		calculateChangesBudgets();
	}
