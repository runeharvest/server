...
{
	/// Constructor
	TPropertyInfoMS(CDataSetMS *ds = NULL, TPropertyIndex propindex = INVALID_PROPERTY_INDEX)
	    : TPropertyInfoBase<CDataSetMS>(ds, propindex)
	{
	}
...
{
	TMessageCarrierWithDestId(bool inputStream)
	    : TMessageCarrier(inputStream)
	{
	}
...
{
public:
	enum TRowManagementTypeMask
	{
		RowManagementBindingCountersBit = 1,
		RowManagementAddingBit = 2,
		RowManagementRemovingBit = 4,
		RowManagementPropChangeBit = 8,
		RowManagementSyncBit = 16 /*extension of AddingBit (set at the same time)*/
	};

	/// Constructor
	CDeltaToMS(CDataSetMS *dataset)
	    : _DataSet(dataset)
	    , _DeltaBuffer(false, false, 0)
	    , _Header(0)
...
public:
	...
	/// Constructor
	CMirrorService()
	    : m_ShardId(DEFAULT_SHARD_ID)
	    , _NumberOfOnlineMS(1)
	    , _RangeManagerReady(false)
	    , _MirrorsOnline(false)
	    , _DeltaSent(false)
	    , _BlockedAwaitingATAck(false)
	    , _EmittedKBytesPerSecond(0.0f)
	    , _EmittedBytesPartialSum(0)
	    , _TimeOfLatestEmittedBytesAvg(0)
	    , _NbDeltaUpdatesReceived(0)
	    , _NbExpectedDeltaUpdates(0)
	    , _IsPureReceiver(false)
	{
		for (uint i = 0; i != 256; ++i)
		{
