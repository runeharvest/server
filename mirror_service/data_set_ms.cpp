...
void CChangeTrackerMS::pushATEAcknowledgesExpectedBeforeUnblocking(TServiceId remoteClientServiceId, const vector<TServiceId> &entityTrackerPairsSentTo)
{
	...
	bool tagGetsWider = (MSInstance->hasTagOfServiceChanged((TServiceId)destServiceId()) && (MSInstance->getPreviousTagOfService(destServiceId()).rawTag() != AllTag));
...
#define KILL_LIST_TYPE(type_name, type)                                   \
	case type_name: {                                                     \
		CMirrorPropValueListMS<type> list(*this, entityIndex, propIndex); \
		list.clear();                                                     \
		break;                                                            \
	}
